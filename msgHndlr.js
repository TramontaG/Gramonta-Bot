...
----------------------\n${clima?.data?.name} - lat: ${clima?.data?.coord?.lat} lon: ${clima?.data?.coord?.lon}
            ...
            case '!cotacao':
                ...
                try {
                    ...
                    let textoSend = `*Nome:* ${coinmarketcapData[parametroBusca[0]].name}\n*Ranking:* ${coinmarketcapData[parametroBusca[0]].cmc_rank != null ? coinmarketcapData[parametroBusca[0]].cmc_rank : 'Sem posição'}\n*Sigla:* ${coinmarketcapData[parametroBusca[0]].symbol}\n*Preço:* ${parseFloat(coinmarketcapData[parametroBusca[0]].quote?.parametroBusca[1]?.price).toLocaleString('pt-br',{style: 'currency', currency: String(`${parametroBusca[1]}`)})}\n*Volume 24h:* ${parseFloat(coinmarketcapData[parametroBusca[0]].quote?.parametroBusca[0]?.volume_24h).toLocaleString('pt-br',{style: 'currency', currency: String(`${parametroBusca[1]}`)})}\n*Suprimento máximo:* ${ coinmarketcapData[parametroBusca[0]].max_supply != null ? parseFloat(coinmarketcapData[parametroBusca[0]].max_supply).toLocaleString('pt-br',{style: 'currency', currency: `${parametroBusca[1]}`}) : 'R$ 0,00'}\n*Suprimento circulante:* ${parseFloat(coinmarketcapData[parametroBusca[0]].circulating_supply).toLocaleString('pt-br',{style: 'currency', currency: String(`${parametroBusca[1]}`)})}\n*Suprimento total:* ${parseFloat(coinmarketcapData[parametroBusca[0]].total_supply).toLocaleString('pt-br',{style: 'currency', currency: String(`${parametroBusca[1]}`)})}\n*Atualização:* ${coinmarketcapData[parametroBusca[0]].quote?.parametroBusca[1]?.last_updated}\n`
