...
----------------------\n${clima?.data?.name} - lat: ${clima?.data?.coord?.lat} lon: ${clima?.data?.coord?.lon}
            ...
            case '!cotacao':
                ...
                try {
                    ...
                    let textoSend = `*Nome:* ${coinmarketcapData[moeda].name}\n*Ranking:* ${coinmarketcapData[moeda].cmc_rank != null ? coinmarketcapData[moeda].cmc_rank : 'Sem posição'}\n*Sigla:* ${coinmarketcapData[moeda].symbol}\n*Preço:* ${parseFloat(coinmarketcapData[moeda].quote?.BRL?.price).toLocaleString('pt-br',{style: 'currency', currency: 'BRL'})}\n*Volume 24h:* ${parseFloat(coinmarketcapData[moeda].quote?.BRL?.volume_24h).toLocaleString('pt-br',{style: 'currency', currency: 'BRL'})}\n*Suprimento máximo:* ${ coinmarketcapData[moeda].max_supply != null ? parseFloat(coinmarketcapData[moeda].max_supply).toLocaleString('pt-br',{style: 'currency', currency: 'BRL'}) : 'R$ 0,00'}\n*Suprimento circulante:* ${parseFloat(coinmarketcapData[moeda].circulating_supply).toLocaleString('pt-br',{style: 'currency', currency: 'BRL'})}\n*Suprimento total:* ${parseFloat(coinmarketcapData[moeda].total_supply).toLocaleString('pt-br',{style: 'currency', currency: 'BRL'})}\n*Atualização:* ${coinmarketcapData[moeda].quote?.BRL?.last_updated}\n`
