...
import DictAPI, { SentenceExample, Syllables, Synonyms, WordMeaning } from "./DictAPI";
...
class Dictionary extends Module {
    ...
    async all(args: Args){
        ...
        try {
            ...
            return Promise.all([syllables, meanings, synoyms, sentenceExamples]).then(messages => {
                ...
                const fullMesage = [
                    ...
                    `${this.createSentenceExampleMessage(word, messages[3] as SentenceExample[])}`,
    ...
    createMeaningMessage(word: string, wordMeanings: WordMeaning[]){
        return wordMeanings.reduce((msg, meaning, index) =>  msg += [
            `*SIGNIFICADO DE ${word.toLocaleUpperCase()}*: ${index + 1}/${wordMeanings.length}`,
            ``,
